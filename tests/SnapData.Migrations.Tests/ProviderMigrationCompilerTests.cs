...
public sealed class ProviderMigrationCompilerTests
{
    ...
    {
        {
            ...
            "app.users",
            "app.accounts",
            ["CREATE TABLE [app].[users]", "[id] BIGINT IDENTITY(1,1) NOT NULL", "[name] NVARCHAR(120)", "DEFAULT 1", "REFERENCES [app].[accounts]", "CONSTRAINT [CK_users_valid] CHECK (1 = 1)", "CREATE INDEX [IX_users_name]"]
        },
        {
            new PostgresMigrationCompiler(),
            "app.users",
            "app.accounts",
            ["CREATE TABLE \"app\".\"users\"", "\"id\" BIGINT GENERATED BY DEFAULT AS IDENTITY NOT NULL", "\"name\" VARCHAR(120)", "DEFAULT TRUE", "REFERENCES \"app\".\"accounts\"", "CONSTRAINT \"CK_users_valid\" CHECK (1 = 1)", "CREATE INDEX \"IX_users_name\""]
        },
        {
            new MySqlMigrationCompiler(),
            "app.users",
            "app.accounts",
            ["CREATE TABLE `app`.`users`", "`id` BIGINT AUTO_INCREMENT NOT NULL", "`name` VARCHAR(120)", "DEFAULT TRUE", "REFERENCES `app`.`accounts`", "CONSTRAINT `CK_users_valid` CHECK (1 = 1)", "CREATE INDEX `IX_users_name`"]
        },
        {
            new FirebirdMigrationCompiler(),
            "users",
            "accounts",
            ["CREATE TABLE \"users\"", "\"id\" BIGINT GENERATED BY DEFAULT AS IDENTITY NOT NULL", "\"name\" VARCHAR(120)", "DEFAULT TRUE", "REFERENCES \"accounts\"", "CONSTRAINT \"CK_users_valid\" CHECK (1 = 1)", "CREATE INDEX \"IX_users_name\""]
        }
    };
    ...
    public static TheoryData<IMigrationCompiler, string, string[]> CheckConstraintCases => new()
    {
        {
            new SqlServerMigrationCompiler(), "app.products",
            [
                "ALTER TABLE [app].[products] ADD CONSTRAINT [CK_products_price] CHECK (price >= 0)",
                "ALTER TABLE [app].[products] DROP CONSTRAINT [CK_products_price]"
            ]
        },
        {
            new PostgresMigrationCompiler(), "app.products",
            [
                "ALTER TABLE \"app\".\"products\" ADD CONSTRAINT \"CK_products_price\" CHECK (price >= 0)",
                "ALTER TABLE \"app\".\"products\" DROP CONSTRAINT \"CK_products_price\""
            ]
        ...
        {
            new MySqlMigrationCompiler(), "app.products",
            [
                "ALTER TABLE `app`.`products` ADD CONSTRAINT `CK_products_price` CHECK (price >= 0)",
                "ALTER TABLE `app`.`products` DROP CHECK `CK_products_price`"
            ]
        },
        {
            new FirebirdMigrationCompiler(), "products",
            [
    ...
    [Theory]
    [MemberData(nameof(CheckConstraintCases))]
    public void Compilers_generate_named_check_constraint_operations(
        IMigrationCompiler compiler,
        string tableName,
        string[] expectedSql)
    {
        ...
        plan.AddCheck(
            tableName,
            new CheckConstraintDefinition("CK_products_price", "price >= 0"));
        plan.DropCheck(tableName, "CK_products_price");
